...
const benefits = [
  "AI-powered ATC radiotelephony simulations",
  "2,500+ aviation vocabulary with pronunciation",
  "ICAO Level 4–6 test preparation",
  "Track progress across all proficiency descriptors",
];
    ...
    <div className="min-h-screen bg-[var(--color-navy-950)] flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-navy-400)] text-sm">Create your account and begin mastering aviation English</p>
        </div>
        ...
        <div className="bg-[var(--color-navy-900)] rounded-2xl border border-[var(--color-navy-800)] p-7">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Capt. Jane Smith" required className="w-full pl-11 pr-4 h-12 bg-[var(--color-navy-800)]/50 border border-[var(--color-navy-700)] text-white placeholder:text-[var(--color-navy-600)] rounded-xl text-sm focus:outline-none focus:border-[var(--color-sky-accent)]" />
              </div>
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-[var(--color-sky-accent)] text-[var(--color-navy-950)] h-12 rounded-xl font-semibold hover:brightness-110 transition disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? (
                <><span className="w-4 h-4 border-2 border-[var(--color-navy-800)]/20 border-t-[var(--color-navy-800)] rounded-full animate-spin" /> Creating account...</>
              ) : (
          ...
          <div className="mt-7 pt-7 border-t border-[var(--color-navy-800)]">
            <ul className="space-y-3">
              {benefits.map((b) => (
                <li key={b} className="flex items-start gap-2.5 text-[var(--color-navy-400)] text-xs">
